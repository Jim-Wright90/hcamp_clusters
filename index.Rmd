...
one_count <- one_impact_test %>% 
  group_by(row, passport_id, gender, age)
...
head(two_impact_test)
  
two_impact_test %>% 
  group_by(gender) %>% 
  summarize(mean(age_post_injury_1))
...
length(unique(one_impact_test$passport_id))
...
length(unique(two_impact_test$passport_id))
...
length(unique(three_impact_test$passport_id))
...
length(unique(four_impact_test$passport_id))

four_test_count <- length(unique(four_impact_test$passport_id))

3400 + 579 + 100 + 11 + 4 + 2

4995 + 742 + 125 + 25 + 5 + 2 + 1

3400 + 4995
579 + 742
125 + 100
8395 + 1321 + 225 + 36 + 9 + 4 + 1
...
scaled_cluster_age_tbl <- function(df, a, b, c) {
  df %>% 
    group_by({{a}}, {{b}}) %>% 
    summarize(Mean = mean({{c}}),
              SD = sd({{c}}),
...
scaled_cluster_age_tbl(clusters_one_impact_test_scaled_long,
                          age,
                          symptom_cluster,
                          post_injury_1_score)
```
...
## Two way anova with age added:

cluster_one_test_aov3 <- aov(post_injury_1_score ~ symptom_cluster * gender * age, 
                            data = clusters_one_impact_test_scaled_long)

summary(cluster_one_test_aov3)

cluster_one_test_aov3

Anova(cluster_one_test_aov3, type = "III")

TukeyHSD(cluster_one_test_aov3)
TukeyHSD(cluster_one_test_aov3, which = "gender")
TukeyHSD(cluster_one_test_aov3, which = "symptom_cluster")
TukeyHSD(cluster_one_test_aov3, which = "age")


res_1a <- TukeyHSD(cluster_one_test_aov3)

tidy(res_1a) %>% 
  reactable()

a_1a <- Anova(cluster_one_test_aov3, type = "III")
